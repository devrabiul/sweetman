    ...
    try {
        ...
        if ($account_type === 'seller') {
            ...
            $next_level  = Level::where('id', '!=', $current_level->id)
                                ...
                                ->whereRaw('? between seller_sales_min and seller_sales_max', [$sales_count])
                                ->latest('id')
                                ->first();
            ...
            $next_level      = Level::where('id', '!=', $current_level->id)
                                    ...
                                    ->whereRaw('? between buyer_purchases_min and buyer_purchases_max', [$purchases_count])
                                    ->latest('id')
                                    ->first();
...
function payment_gateway($slug, $update = false)
{
    try {
        ...
        if ($update) {
            ...
            // Return data
            return Cache::rememberForever('payment_gateway_settings_' . $slug, function () use ($slug) {
