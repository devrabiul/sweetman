...
@if ($user->account_type === 'seller' && $last_delivery)
    ...
    <div class="flex justify-between w-full">
            <span class="text-gray-700 font-semibold text-sm">@lang('messages.t_last_delivery')</span>
...
<li class="bg-white shadow-md rounded-lg p-4 flex items-center space-x-3">
    ...
    <span class="text-gray-700 font-semibold text-sm">@lang('messages.t_member_since') {{ format_date($user->created_at, config('carbon-formats.F_j,_Y')) }}</span>
</li>
...
<div class="flex flex-col items-center gap-8 p-6 bg-transparent rounded-lg shadow-lg">
    ...
    <div class="flex items-center gap-2">
        {!! render_star_rating($user_rating['value'], "1rem", "1rem", "#d0d0d0") !!}
...
{{-- Total reviews --}}
@if($user_rating['total'] > 1)
    <span class="text-md text-slate-400">Based on {{ $user_rating['total'] }} reviews</span>
@endif
            ...
            <div class="flex flex-col w-full gap-4">
                @foreach([5, 4, 3, 2, 1] as $star)
                    <div class="flex items-center w-full">
                        <label class="flex-none w-16 text-md text-slate-500 dark:text-zinc-400">{{ $star }} stars</label>
                        <div class="flex-1 bg-gray-200 rounded-full h-3 overflow-hidden">
                            <div class="bg-red-500 h-full" style="width: {{ $user_rating['stars_'.$star] * 100 / $user_rating['total'] }}%;"></div>
                        </div>
                        <span class="flex-none w-12 text-md font-bold text-slate-700 dark:text-zinc-300">{{ $user_rating['stars_'.$star] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="bg-white shadow-sm shadow-gray-100 border border-slate-100 dark:border-transparent dark:shadow-none rounded-lg dark:bg-zinc-800 col-span-2 py-6 divide-y dark:divide-zinc-700 space-y-8">
                ...
                @if ($gigs && $gigs->count())
                    <section class="px-7 pt-8">
                        ...
                        {{-- Section title --}}
                        <div class="mb-5">
                            <h1 class="text-base font-extrabold text-zinc-800 tracking-wide dark:text-zinc-100">@lang('recent offers')</h1>
                        </div>
...
{{-- Thumbnail --}}
                                         <a href="{{ url('service', $gig->slug) }}" class="flex-none md:w-32 h-min rounded-lg overflow-hidden relative">
    <img src="{{ placeholder_img() }}" data-src="{{ src($gig->thumbnail) }}" alt="{{ $gig->title }}" class="rounded-lg object-cover w-full h-full transition duration-500 lazy">
</a>
